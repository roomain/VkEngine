...
void Resources::emplace(BinHeader&& a_header, const Binary& a_binary)
{
    ...
    {
        m_headings.emplace_back(std::move(a_header));
    }
...
std::vector<Resources::Delta> Resources::diff(const Resources& a_resources)const
{
    std::vector<Delta> changeList;

    // get updated and new
    for (const auto& header : a_resources.m_headings)
    {
        if (auto iter = std::ranges::find_if(m_headings, [&header](auto&& curHeader) {
            return std::string(curHeader.filename.data()) == std::string(header.filename.data()); });
    ...
    {
        if (auto iter = std::ranges::find_if(a_resources.m_headings, [&header](auto&& curHeader) { return std::string(curHeader.filename.data()) == std::string(header.filename.data()); });
            iter == a_resources.m_headings.cend())
        {
            changeList.emplace_back(
                header,
